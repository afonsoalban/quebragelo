...
const questionsState = {
    available: {
        easy: [...originalQuestions.easy],
        hard: [...originalQuestions.hard]
    },
    asked: 0
};

const hasQuestionsLeft = () => (
    questionsState.available.easy.length > 0 || questionsState.available.hard.length > 0
);

const getRandomIndex = (size) => Math.floor(Math.random() * size);

const drawQuestion = (category) => {
    const pool = questionsState.available[category];
...
const getFallbackCategory = (category) => (category === 'hard' ? 'easy' : 'hard');

document.addEventListener('DOMContentLoaded', () => {
    const questionElement = document.querySelector('h1');
    const actionButton = document.querySelector('[data-action="new-question"]');

    const showNoQuestions = () => {
        questionElement.textContent = 'Acabaram as perguntas! Volte mais tarde.';
    ...
    const showQuestion = () => {
        ...
        const preferredCategory = (questionsState.asked + 1) % 5 === 0 ? 'hard' : 'easy';
        let question = drawQuestion(preferredCategory);

        if (!question) {
            const fallbackCategory = getFallbackCategory(preferredCategory);
            question = drawQuestion(fallbackCategory);
        }
        ...
        questionsState.asked += 1;
        questionElement.textContent = question;
    };

    actionButton.addEventListener('click', showQuestion);
    showQuestion();
});
